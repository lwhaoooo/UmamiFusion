...
class GATBranchWithAttention(nn.Module):
    def __init__(self, num_features_xd=320, node_dim=128, output_dim=128,
        ...
        # GAT图卷积层
        self.gcn1 = GATConv(num_features_xd, num_features_xd, heads=gat_heads, dropout=dropout)
        self.gcn2 = GATConv(num_features_xd * gat_heads, output_dim, dropout=dropout)

        # 全连接层
        self.fc_g1 = nn.Linear(output_dim, node_dim)
    ...
    def forward(self, x, edge_index):
        ...
        Returns:
            features: [total_nodes, node_dim] 节点级特征
        """
        # GAT层1
        # pdb.set_trace()
...
class MultiTaskFusionNetWithAttention(nn.Module):
    ...
    def forward(self, data, return_features=False):
        # 提取特征
        graph_node_features = self.gat_branch(data.x, data.edge_index)
        ...
        # 对图的注意力输出进行池化
        graph_attended_flat = graph_attended.view(-1, self.gat_branch.node_dim)

        # 重新构建batch索引（每个样本39个节点）
        batch_size = graph_attended.size(0)
...
# 工厂函数：创建模型实例
# ============================================
def GATConvNet(num_features_xd=320, num_features_xt=25, n_output=1, gat_heads=5):
    """
    工厂函数：创建带交互注意力的GAT模型
    """
    # 超参数配置
    ...
    # 创建GAT图分支
    gat_branch = GATBranchWithAttention(
